...
class YouTube:
	...
	def get_upload_data(self, video_path):
		metadata = {
			...
			'keywords'     : ', '.join(YOUTUBE_GENERIC_TAGS + filename(rmext(video_path)).split()),
		# adds each word of the audio file (without extension) as a tag
	...
	def get_video_title(self, video_path=None):
		return scheme(YOUTUBE_TITLE_SCHEME, {
			...
			'title'     : self.get_track_title(video_path) if video_path is not None else CLI_STYLING_CODES['DARK_RED'] +
																						  'track title' +
																						  CLI_STYLING_CODES['ENDC'],
